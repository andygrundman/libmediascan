...
static void
buf_dst_mgr_init(j_compress_ptr cinfo)
{
...
static boolean
buf_dst_mgr_empty(j_compress_ptr cinfo)
{
  struct buf_dst_mgr *dst = (void *)cinfo->dest;
  void *tmp;
...
static void
buf_dst_mgr_term(j_compress_ptr cinfo)
{
  struct buf_dst_mgr *dst = (void *)cinfo->dest;
...
static void
image_jpeg_buf_dest(j_compress_ptr cinfo, struct buf_dst_mgr *dst)
{
...
int
image_jpeg_load(MediaScanImage *i, MediaScanThumbSpec *spec_hint)
{
  float scale_factor;
  int x, w, h, ofs;
...
void
image_jpeg_compress(MediaScanImage *i, MediaScanThumbSpec *spec)
{
...
static void
parse_exif_ifd(ExifContent *content, void *data)
{
  exif_content_foreach_entry (content, parse_exif_entry, data);
}

static void
parse_exif_entry(ExifEntry *e, void *data)
{
