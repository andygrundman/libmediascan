...
fileSystemRepresentationWithPath:path], &fileInfo) < 0) {
  ...
  while (S_ISLNK(fileInfo.st_mode) ||
         (!S_ISDIR(fileInfo.st_mode) && (aliasTarget =[path stringByConditionallyResolvingAlias]) != nil)) {
...
-(NSString *)stringByConditionallyResolvingSymlink {
  ...
  // Resolve the symlink final component in the path
  //
  NSString *symlinkPath =[[NSFileManager defaultManager]
destinationOfSymbolicLinkAtPath: self error:NULL];
  if (!symlinkPath) {
    return nil;
  }
if (![symlinkPath hasPrefix:@"/"]) {
    ...
    // relative links
    //
    symlinkPath =[[self stringByDeletingLastPathComponent]
  ...
  if (url != NULL) {
    ...
    if (CFURLGetFSRef(url, &fsRef)) {
      ...
      if ((err == noErr) && wasAliased) {
        ...
        if (resolvedUrl != NULL) {
          resolvedPath =[(id) CFURLCopyFileSystemPath(resolvedUrl, kCFURLPOSIXPathStyle)
                         autorelease];
